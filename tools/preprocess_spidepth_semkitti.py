#!/usr/bin/env python3
"""Generate SPIdepth predictions for SemanticKITTI SSC frames.

The script follows SPIdepth's official ``test_simple_SQL_config.py`` protocol:
320x1024 RGB input, horizontal-flip averaging, bilinear resizing back to the
source resolution, and float32 metric-depth ``*_disp.npy`` outputs.
"""
...
def parse_args():
    ...
    parser.add_argument('--data-root', required=True,
                        help='SemanticKITTI dataset directory containing sequences/')
    parser.add_argument('--output-root', required=True,
                        help='Output directory containing sequences/<id>/*.npy')
    ...
    parser.add_argument('--all-images', action='store_true',
                        help='Process every image instead of SSC voxel-labelled frames')
...
def collect_items(args):
    ...
    for sequence in args.sequences:
        ...
        if args.all_images:
            frame_ids = sorted(p.stem for p in (sequence_root / 'image_2').glob('*.png'))
        else:
            frame_ids = sorted(p.stem for p in (sequence_root / 'voxels').glob('*.bin'))
        for frame_id in frame_ids:
            image_path = sequence_root / 'image_2' / f'{frame_id}.png'
            output_path = output_root / 'sequences' / sequence / f'{frame_id}_disp.npy'
...
def main():
    ...
    print(f'Queued {len(items)} frames from sequences {args.sequences}', flush=True)
    if not items:
        return
        ...
        for images, original_sizes, output_paths in loader:
            ...
            for prediction, original_size, output_path in zip(
                ...
                prediction = F.interpolate(
                    prediction[None], size=tuple(original_size), mode='bilinear',
