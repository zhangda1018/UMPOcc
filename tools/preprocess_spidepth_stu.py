#!/usr/bin/env python3
"""Generate SPIdepth metric-depth maps for VAA-STU frames."""
...
def collect_items(args):
    ...
    for sequence in args.sequences:
        ...
        frame_ids = sorted(
            path.stem for path in (sequence_root / 'voxels').glob('*.label'))
        if not frame_ids:
            raise FileNotFoundError(
                'No voxel labels found under {}'.format(sequence_root / 'voxels'))

        for frame_id in frame_ids:
            image_path = (
                sequence_root / 'image' / 'port_a_cam_0' /
                '{}.png'.format(frame_id))
            output_path = (
                output_root / sequence / '{}_disp.npy'.format(frame_id))
...
def main():
    ...
    print(
        'Queued {} VAA-STU frames from sequences {}'.format(
            len(items), args.sequences),
        flush=True)
    if not items:
        return

    loader = DataLoader(
        ImageDataset(items, (opt.width, opt.height)),
    ...
    with torch.inference_mode():
        for images, original_sizes, output_paths in loader:
            ...
            predictions = 0.5 * (
                normal + torch.flip(flipped, dims=(-1,)))
            ...
            if (completed % args.log_interval < len(output_paths) or
                    completed == len(items)):
                elapsed = time.time() - started
                print(
                    'Processed {}/{} frames ({:.2f} frame/s)'.format(
                        completed, len(items),
                        completed / max(elapsed, 1e-6)),
                    flush=True)
